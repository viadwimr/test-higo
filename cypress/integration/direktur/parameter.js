...
describe('Parameter', () => {
  ...
  describe('Filter', () => {
    ...
    it('Filter nama', () => {
      cy.wait(3000);
      cy.get('[data-testid=sector] > .ant-select-selector', timeout).click();
      cy.contains('Semua Sektor', timeout).click();
      cy.get('[data-testid=search-device]', timeout).type('air');
      cy.contains('air blower c', timeout).should('be.visible');
      cy.contains('air blower p', timeout).should('be.visible');
      cy.contains('Zona 2 ST12 Air Temperature', timeout).should('be.visible');
      cy.contains('Zona 3 ST13 Air Temperature', timeout).should('be.visible');

      cy.get('[data-testid=sector] > .ant-select-selector', timeout).click();
      cy.get('input[id="sector_search"]', timeout).type('forming');
      cy.contains('Forming Baking Line 2', timeout).click();
      cy.get('[data-testid=search-device]', timeout).clear().type('spri');
      cy.contains('Sprinkle wiremesh_speed', timeout).should('be.visible');
    });
  });
