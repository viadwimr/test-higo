...
describe('Report', () => {
  ...
  describe('PARAMETER', () => {
    ...
    it('Shiftly', () => {
      ...
      cy.contains('Shift Pagi', timeout).click();
    ...
    it('Filter asset', () => {
      cy.get('#parameter_device_id', timeout).click();
      cy.get('div[class="ant-select-tree-treenode ant-select-tree-treenode-disabled ant-select-tree-treenode-switcher-open"]', timeout).should('exist');
    });
  });
  ...
  describe('Losses', () => {
    beforeEach(() => {
      ...
      cy.contains('Availability losses', timeout).click();
      cy.get(':nth-child(2) > [data-testid=asset-picker] > .ant-select-selector > .ant-select-selection-search > #line_machine', timeout)
        .type('Packaging Line 1');
      cy.contains('Packaging Line 1', timeout).click();
      cy.get(':nth-child(3) > .CustomPopup__Container-tuxi8-0 > [data-testid=date-root]', timeout).click();
      cy.get('button[class="Picker__Option-sc-1xytywo-4 bzjWVz"]', timeout).eq(4).click(force);
      cy.get('[data-testid=submit-btn-losses]', timeout).click();
    });
    describe('Sorting', () => {
      it('Ascending', () => {
