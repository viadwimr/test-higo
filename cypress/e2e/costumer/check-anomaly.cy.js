...
describe('Usage Pelanggan', () => {
  ...
  beforeEach(() => {
    cy.visit('/');
    cy.get('#username', timeout).type('check');
  ...
  it('Detect anomaly', () => {
    cy.get(':nth-child(8) > .v-list-item__content', timeout).click();
    ...
    while(monthLoop<=thisMonth+1) {
      ...
      if(9 + thisMonth - monthLoop>=9) {
        cy.get('.vjsf-property-bulan > .v-input > .v-input__control', timeout).click();
        cy.wait(1000);
        cy.get('.v-list-item__content > .v-list-item__title', timeout).eq(9 + thisMonth - monthLoop).click();
      }
      monthLoop++;
    }
  })
  ...
  it('List anomaly', () => {
    ...
    while(monthLoop <= thisMonth+1) {
      ...
      cy.get('.v-data-footer__select > .v-input > .v-input__control > .v-input__slot > .v-select__slot', timeout).click();
      cy.wait(1000);
      cy.get('.v-list-item__content > .v-list-item__title', timeout).eq(8).click();
      cy.wait(5000);
