...
describe('Indicator', () => {
  ...
  describe('Cek Data Indicator', () => {
    ...
    it('Kelembapan', () => {
      cy.get('form > :nth-child(1) > label', timeout).click({ force: true });
      cy.get('#indikator_form_sensor_name', timeout).should('have.value', 'Kelembapan');
      cy.get('#indikator_form_sensor_unit', timeout).should('have.value', '%');
      cy.get('#indikator_form_min_valid', timeout).should('have.value', '0');
      cy.get('#indikator_form_max_valid', timeout).should('have.value', '200');
    });

    it('Temperatur', () => {
      cy.get('form > :nth-child(2) > label', timeout).click({ force: true });
      cy.get('#indikator_form_sensor_name', timeout).should('have.value', 'Temperatur');
      cy.get('#indikator_form_sensor_unit', timeout).should('have.value', '℃');
      cy.get('#indikator_form_min_valid', timeout).should('have.value', '0');
  ...
  describe('Sorting Data Indicator', () => {
    ...
    it('Sorting List Indicator', () => {
      cy.contains('1. Kelembapan (%)', timeout).should('be.visible');
      cy.contains('2. Temperatur (℃)', timeout).should('be.visible');
    });
  });
  ...
  describe('Search Indicator', () => {
    ...
    it('Data found', () => {
      cy.get('#sector_search', timeout).type('Kelemba{enter}');
      cy.get('label', timeout).should('contain.text', '1. Kelembapan (%)');
      cy.get('form > :nth-child(1) > label', timeout).click({ force: true });
      cy.get('#indikator_form_sensor_name', timeout).should('have.value', 'Kelembapan');
      cy.get('#indikator_form_sensor_unit', timeout).should('have.value', '%');
      cy.get('#indikator_form_min_valid', timeout).should('have.value', '0');
